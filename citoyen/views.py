...
def download_tdr_pdf_view(request):
...
<div class="page-break">
    ...
    <ul>
        ...
        <li>
            ...
            <br>&bull; Assurer la traçabilité complète de <strong>100%</strong> des actions d'approbations et de rejets avec enregistrement des motifs.
        </li>
        <li>
...
<div class="page-break">
    ...
    <ul>
        <li><strong>Normes de codage de haut niveau :</strong> Respect absolu de la directive PEP 8 pour l'écriture de l'ensemble du code Python de l'infrastructure. Typage fort des contrôles et refus des lignes abrégées ou tournantes.</li>
        ...
        <li><strong>Critères d'homologation :</strong> Zéro vulnérabilité d'isolation de rôles constatée lors des audits de sécurité. Validation systématique du format d'impression de la carte d'identité sur le PDF généré par les contrôleurs administratifs.</li>
    </ul>
</div>
...
"""
    ...
    response['Content-Disposition'] = 'attachment; filename="TERMES_DE_REFERENCE.pdf"'
    
    pisa_status = pisa.CreatePDF(html_content, dest=response)
    
    if pisa_status.err:
        return HttpResponse('<h1>Erreur lors de la génération du document PDF</h1>', status=500)
...
def download_cin_pdf_view(request, pk):
    ...
    uniquement lorsque son dossier a été validé.
    """
    ...
    # Sécurité stricte : disponible uniquement si validé
    if cin.statut != 'VALIDE':
        return HttpResponseForbidden("Téléchargement non autorisé : Le dossier de CIN n'est pas encore validé.")
        
    template_path = 'controlleur/pdf_card.html'
    context = {'cin': cin}
    
    response = HttpResponse(content_type='application/pdf')
    response['Content-Disposition'] = f'attachment; filename="CIN_OFFICIEL_{cin.numero_cin}.pdf"'
    ...
    if pisa_status.err:
        return HttpResponse("Erreur fatale lors de la génération du document PDF de votre CIN.", status=500)
